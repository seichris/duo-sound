...
**Verified public API and runtime:** Apple demonstrates SwiftUI `onHingeChange`, nullable `context.hinge`, `hinge.angle` (SwiftUI `Angle`), and `.partiallyOpen`; the same talks describe UIKit `UIHingeInteraction`. The app's Duo adapter installs the public UIKit interaction through a runtime bridge, allowing a stable-SDK distribution binary to run on iOS 27.1 while older systems report no hinge. The bridge does not invent a `HingeManager`, device model identifier, permission, or background service. Initial delivery, angle semantics at endpoints and lifecycle ordering have been exercised in Apple's Duo simulator; physical-device behavior still needs separate validation.

**Availability:** Xcode 27.1 beta (build 27A9269) includes the iOS 27.1 SDK, while the submitted candidate is built with stable Xcode 26.6 (build 17F113). The default build still excludes the adapter at compile time, while the Duo schemes enable it. `#available` plus class lookup keeps the stable binary installable on older iOS versions without linking an unavailable SDK symbol.
