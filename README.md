...
**Local validation:** 29 XCTest tests passed on Swift 6.2.1/Linux; all Swift files parsed, including the opt-in adapter branch. Linux cannot type-check SwiftUI/AVFoundation or run Xcode. CI status is separate evidence. The current app has no final App Store icon or signed distribution archive.
