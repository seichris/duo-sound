...
> **September 14, 2026: developer preview, not a working system-wide fold-sound utility.** Apple has publicly demonstrated `onHingeChange`, but its Duo developer page still lists Xcode 27.1 beta as coming later this month. The default scheme has **simulated folds only**. The opt-in hinge adapter follows Apple's published sample and is not yet SDK-compiled or hardware-validated. Foreground only; no background/lock-screen listener is claimed. [Research and sources](docs/IPHONE_DUO_RESEARCH.md).
...
After Apple actually ships the required SDK, inspect its declarations and select **DuoSound-DuoSDK**. That scheme sets `DUO_HINGE_SDK`; the default scheme does not. This is a compile-time gate, not a hidden runtime API lookup. Follow [the SDK/device validation checklist](docs/TESTING.md) before claiming automatic support or making a release. Do not enable this scheme in an older SDK: a compile error is expected.
...
xcodebuild -project DuoSound.xcodeproj -scheme DuoSound \
...
**Local validation:** 39 XCTest tests passed on Swift 6.2.1/Linux; all Swift files parsed, including the opt-in adapter branch. Linux cannot type-check SwiftUI/AVFoundation or run Xcode. CI status is separate evidence. The App Store preparation branch adds an original generated app icon; there is still no signed distribution archive.
...
See [the sourced SEO/ASO research](docs/ASO_SEO_RESEARCH.md) and [the release guide](docs/APP_STORE_RELEASE.md). Proposed listing: **Duo Sound: Fold Effects** / **Custom Open & Close Audio**. Preparation includes English US/UK copy, an original generated icon, in-app privacy/help, a static noindex preview site, screenshot capture tooling, UI smoke tests and guarded native archive/export. No account details, store identifiers, search metrics or hardware validation are invented.
...
The **Release preparation** workflow publishes a reviewable ZIP artifact; **CI** runs real simulator UI tests and retains raw screenshot attachments. Neither is an App Store upload or physical-Duo validation. The dedicated **DuoSound-AppStore** configuration and shared Archive actions require the release gate. The app icon is generated automatically before Xcode compiles assets, rather than stored as an opaque hand-maintained binary. No site is deployed by this workflow.
