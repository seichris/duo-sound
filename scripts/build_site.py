...
contact=(f'<p>Publisher: {esc(owner["legal_name"])}. <a href="mailto:{esc(owner["support_email"],quote=True)}">Email support</a>.</p>'
         ...
         '<p>Development support: <a href="https://github.com/seichris/duo-sound/issues">the Hingy project on GitHub</a>. Publisher/contact details must be configured before public release. Issues are public; do not share private information.</p>')
summary=(rc.ROOT/'App/PrivacySummary.txt').read_text()
privacy=''.join('<p>'+esc(p)+'</p>' for p in summary.split('\n\n')[:-1])+contact
pages={
    'index.html': ('Hingy — opening and closing effects for iPhone Duo',
        ...
        '''<div class="hero"><img src="assets/app-icon.png" width="144" height="144" alt="Hingy: an original folded-waveform icon"><div><p class="eyebrow">HINGY</p><h1>A little sound.<br>Your kind of fold.</h1><p class="lead">Choose an opening effect. Give closing its own voice. Start with original sounds or import a short clip.</p></div></div>
        <section><h2>Opening and closing sounds for iPhone Duo</h2><p>The intended physical-fold experience works only while Hingy is visible and active on supported hardware. It is not an iOS system-sound setting or an always-on background service.</p><p>In the developer preview, Open, Close, and the hinge slider are clearly labelled simulations. Physical hinge support still needs SDK compilation and device validation.</p></section>
        <div class="grid"><section><h2>Make it yours</h2><p>Choose from 20 original sounds across Musical, Mechanical, Nature, Playful, and Sci-Fi categories. Search the library and preview Crystal, Zipper, Bird, Bubble, Laser, and more without changing your saved selection. Choose independently for each direction. Import playable unprotected audio up to 5 seconds and 8 MiB.</p></section><section><h2>Keep control</h2><p>Adjust effect volume, disable either effect, or preview a selection. Playback respects Silent Mode and follows your current audio output.</p></section></div>
